...
**构建状态：** ![tests](https://img.shields.io/badge/tests-366-brightgreen) ![analyze](https://img.shields.io/badge/analyze-0%20issues-brightgreen) CI 自动构建 → Release 发布（ZIP 便携版 + Setup.exe 安装包 + split APK）
...
| v0.4.0 | 2026-07-15 | 架构重构 + DesignTokens + 340 测试 |
| v0.4.1 | 2026-07-15 | MinerU fallback + 错误处理优化 |
| v0.4.2 | 2026-07-15 | 划词问答 + 文库排序/搜索 + batch import + dark mode + 重构 |
| v0.4.3 | 2026-07-15 | API 服务器 + 收藏/星标 + 断点续读 + 字体持久化 + MinerU Agent 降级 + Zotero 导入 + 高亮笔记 |

### 核心能力一览

| 能力 | 技术方案 |
|---|---|
| PDF 解析 | MinerU v4 API（异步提交 → 轮询 → 下载 ZIP）→ Agent API 降级 → poppler → flutter_pdf → 元数据 |
...
| DI 容器 | `ServiceLocator`（`lib/core/di/service_locator.dart`） |
| 设计 Token | `DesignTokens`（`lib/core/tokens/design_tokens.dart`，4px 网格，0 硬编码） |
| 共享初始化 | `createLocator()`（`lib/core/init.dart`，CLI + Flutter 共用） |
| 测试替身 | `mock_services.dart`（11 个 mock 服务，`test/helpers/`） |
| REST API | shelf + shelf_router（`lib/server_main.dart`，Flutter 入口独立启动） |
| 收藏/星标 | `Paper.starred` 字段 + 文库 AppBar 切换筛选 |
| 断点续读 | `Paper.scrollPosition` 字段，打开论文时自动恢复滚动位置 |
| PDF 降级链 | MinerU API → Agent API（免 Key，≤20页/10MB）→ poppler → flutter_pdf → 元数据 |
| Zotero 集成 | `ZoteroApi` 客户端 + 导入对话框（从 Zotero 库导入论文） |
| Deep link | `paperpal://arxiv/XXXX` 协议 + 安装时注册（Windows） |
...
│   ├── server_main.dart                   # [v0.4.3] API 服务器入口（flutter run -t lib/server_main.dart）
│   │
...
│   │   │   └── zotero_api.dart            # [v0.4.3] Zotero 库 API 客户端
│   │   │
│   │   ├── models/                        # 数据模型（8 文件）
│   │   │   ├── app_error.dart
│   │   │   ├── config.dart                # AppConfig（纯 Dart，Flutter extension 移至 ui/）
│   │   │   ├── note.dart                  # 笔记（含 NoteType: note / highlight / question）
│   │   │   ├── paper.dart                 # 论文（含 toJson/fromJson、starred、scrollPosition）
│   │   │   ├── parse_result.dart
│   │   │   ├── search_result.dart
│   │   │   ├── soul.dart                  # 灵魂定义
...
│   │   │   ├── pdf_fallback_service.dart  # [v0.4.3 重写] PDF 降级链：Agent API → poppler → flutter_pdf → 元数据
...
│       │   ├── library_page.dart          # 论文库（含花色标记、金色装饰线、错列入场、星标筛选）
...
│       │
│       ├── widgets/                       # 组件（12 文件）
│       │   ├── animated_background.dart   # [v0.2.0] 动感渐变背景（Canvas 3 点径向漂移 + 花色暗纹，含 RepaintBoundary 优化）
...
│       │   ├── notes_panel.dart           # [v0.4.3] 笔记面板（从 read_page 提取，支持高亮/笔记/问答类型）
│       │   ├── page_transition.dart       # [v0.2.0] 自定义页面过渡（cubic 贝塞尔滑入）
│       │   ├── progress_bar.dart          # [v0.2.0] 阅读进度条（3px 金色渐变）
│       │   ├── qa_panel.dart              # [v0.4.3] 问答面板（从 read_page 提取，动态高度 + Shift+Enter 发送）
│       │   ├── skeleton_loader.dart       # [v0.2.0] 骨架屏（呼吸闪烁占位）
│       │   └── soul_selector.dart         # 灵魂选择器（金色选中态）
│       │
│       └── theme/
│           └── app_theme.dart             # [v0.2.0 重写] 完整双主题 ColorScheme + 自定义 TextTheme/CardTheme/InputTheme/ButtonTheme
│
├── test/                                  # 测试（24 文件，366 测试）
│   ├── core/
...
│   └── widget_test/                       # [v0.4.3] Widget 测试（7 测试）
│       ├── avatar_picker_test.dart        # 头像选择器打开/关闭（2）
│       ├── explain_dialog_test.dart       # 公式解释弹窗渲染（1）
│       └── soul_selector_test.dart        # 灵魂选择/切换/高亮/空状态（4）
│
...
│   └── key.properties.example            # [v0.4.3] Android 签名配置模板
│
├── windows/                              # Windows 平台文件
│   ├── runner/main.cpp                    # 入口（含文件关联参数传递 + Deep link 协议注册）
...
通过 `ServiceLocator` 轻量 DI 容器管理。所有服务在 `createLocator()` 中注册一次，通过 `Dependencies` InheritedWidget 注入 widget 树。

```dart
// lib/core/init.dart — 共享初始化
...
runApp(PaperPalApp(locator: locator));

// 任意 widget 中访问（通过 ServiceX 扩展）：
context.paperService.askQuestionStream(...);
context.soulService.getActiveOrDefault();

// 或通过 locator（CLI 模式）：
locator.get<IPaperService>().askQuestionStream(...);
```

**CLI 和 Flutter 共用同一初始化逻辑**，见 `lib/core/init.dart`。

注册的 11 个服务（均通过接口类型访问）：`IConfigService`, `ICacheService`, `ILLMProvider`, `ISoulService`, `IMemoryService`, `IPortraitService`, `IAvatarService`, `ISearchService`, `INetworkService`, `INoteService`, `IPaperService`。
    ...
    └── _AppShellState.build()
        ...
        └── AnimatedBackground    ← animated_background.dart（渐变层 + 花色暗纹）
            ...
            └── IndexedStack
                ...
                ├── LibraryPage    ← library_page.dart（花色标记 + 金色装饰线 + 星标筛选）
...
- v0.4.3 优化：子组件包裹 `RepaintBoundary`，避免每帧重绘整棵树
...
| **LibraryPage** | 花色标记（♠♥♦♣ 基于 paper.id.hashCode）、金色 3px 左侧装饰线、错列入场动画、骨架屏、星标筛选切换 |
...
| Zotero | `GET /users/{userId}/items/top` | 论文库导入 | 根据 API Key 配额 |
...
| `syncfusion_flutter_pdf` | any | PDF 页数检测 + flutter_pdf 回退解析 | 跨平台 |
...
| `shelf` | any | HTTP 服务器（API 模式） | 跨平台 |
| `shelf_router` | any | REST 路由 | 跨平台 |
| `shelf_cors_headers` | any | CORS 中间件 | 跨平台 |
...
| 方法 | 职责 | 涉及的外部依赖 |
|---|---|---|
| `importPdf(File)` | 本地 PDF 导入 → 保存到缓存 | `CacheService`, `MineruApi`, `PdfFallbackService` |
| `importUrl(String)` | URL 论文导入 → 自动下载 PDF | `CacheService`, `MineruApi`, `ArxivApi`（补全元数据）, `PdfFallbackService` |
| `importSearchResult(SearchResult, String)` | 搜索结果导入 → 下载 PDF | `SearchService`, `CacheService` |
| `parsePaper(String)` | 触发 MinerU 解析 → 轮询 → 失败降级 → 获取结果 | `MineruApi`, `ParseService`, `PdfFallbackService` |
...
| `translatePaper(String)` | 全文 AI 译文 | `TranslationService`, `LLMProvider` |
...
### 5.3 PdfFallbackService — PDF 降级链（`lib/core/services/pdf_fallback_service.dart`）

v0.4.3 重写。当 MinerU 标准 API 解析失败时，按以下优先级逐级降级：

1. **MinerU Agent API**（`/api/v1/agent/parse/file`，免 Key，限制 ≤20页/10MB）
2. **poppler**（`pdftotext` 命令行，需系统安装）
3. **flutter_pdf**（`syncfusion_flutter_pdf` 内置提取）
4. **元数据**（仅提取文件名+标题，0 页内容）

每一级失败自动进入下一级，最终保证至少返回一个包含元数据的 `ParseResult`。

### 5.4 ZoteroApi — Zotero 集成（`lib/core/api/zotero_api.dart`）

v0.4.3 新增。通过 Zotero API 从用户个人库中拉取论文列表，支持分页和按条目类型（期刊/预印本/会议/书籍章节/学位论文）过滤。返回的数据映射为 `SearchResult`，可直接用于导入流程。

```dart
class ZoteroApi {
  Future<List<SearchResult>> listItems({int limit = 50, int start = 0});
}
```

需要环境变量 `ZOTERO_API_KEY` 和 `ZOTERO_USER_ID`。

### 5.5 REST API 服务器（`lib/server_main.dart` / `tool/server.dart`）

v0.4.3 新增。基于 `shelf` + `shelf_router` 的 REST API 服务器，以独立 Flutter entry point 运行：

```bash
flutter run -t lib/server_main.dart --dart-define=PORT=4090
# 或纯 Dart 模式：
dart run tool/server.dart
```
...
| POST | `/papers/:id/ask` | AI 问答（SSE 流式） |
| POST | `/papers/:id/summarize` | 生成摘要 |
| GET | `/souls` | 列出灵魂 |
| POST | `/souls/active` | 设置活跃灵魂 |
| GET | `/notes?paperId=` | 获取笔记 |
...
**当前 366 个测试，覆盖全部纯逻辑层 + Widget 交互 + 端到端管线：**
...
### Widget 测试（7 个，v0.4.3 新增）

| 测试文件 | 测试数 | 覆盖范围 |
|---|---|---|
| `test/widget_test/avatar_picker_test.dart` | 2 | 头像选择器打开/关闭交互 |
| `test/widget_test/explain_dialog_test.dart` | 1 | 公式解释弹窗渲染 |
| `test/widget_test/soul_selector_test.dart` | 4 | 灵魂选择/切换/高亮/空状态 |

### flutter analyze

当前 **0 issues**，v0.4.3 已清零以下 lint 类别：
- `use_build_context_synchronously`（18→0）
- `test/` 中 `const` constructors（125→0）
- CLI 中 `print()` → `stdout.writeln`

**未覆盖领域：**
- 页面（SearchPage / ReadPage / SettingsPage / LibraryPage）— 部分已有 smoke test
...
| 12 | AnimatedBackground 性能 | Canvas CustomPainter 每帧重绘，在低配机器上可能增加 CPU 占用。v0.4.3 已添加 RepaintBoundary 优化 |
...
| 21 | MinerU Agent API 自动降级 | Agent API（`/api/v1/agent/parse/file`）已在 v0.4.3 中实现为 PdfFallbackService 的第一降级级。免 Token，限制 ≤20页/10MB，仅输出 Markdown |
| 22 | Zotero API 需要环境变量配置 | ZOTERO_API_KEY / ZOTERO_USER_ID 通过环境变量传入，无 UI 配置入口 |
| 23 | Deep link 仅支持启动时传入 | `paperpal://arxiv/XXXX` 协议仅在应用启动时通过命令行参数解析，不支持运行时拦截 |
...
### 为什么使用 shelf 而非 Flask/FastAPI 做 API 服务器？
- 复用 Dart 生态，零语言切换成本
- `ServiceLocator` DI 容器 + `lib/core/` 纯 Dart 代码可直接复用
- 与 Flutter 共享同一套数据模型和业务逻辑，无重复定义
...
|---|---|---|
| Phase 3+ | iOS 适配 | 待定 |
| Phase 3+ | PDF 标注/高亮（真正的 PDF 内置标注） | 待定 |
| — | 更多 LLM Provider（本地 Ollama） | 1 天 |
| — | Zotero 集成（完善 UI 配置） | 2-3 天 |
| — | 引用网络可视化 | 待定 |
| — | Android 自适应横屏布局 | 1 天 |
| — | 自动 APK 签名配置 | 1 天 |
...
### v0.4.3（2026-07-15）— API Server + 收藏/星标 + 断点续读 + MinerU 降级 + Zotero + 高亮笔记

**REST API 服务器：**
- `lib/server_main.dart` — 基于 shelf + shelf_router 的 Flutter entry point
- 端点：论文 CRUD、AI 问答(SSE)、灵魂切换、笔记查询
- 启动方式：`flutter run -t lib/server_main.dart` 或 `dart run tool/server.dart`
- `b26830b` refactor: extract QAPanel + NotesPanel widgets (read_page 1000→575 lines)

**收藏/星标系统：**
- `Paper.starred` 布尔字段，含 toJson/fromJson 序列化
- 文库 AppBar 新增星标筛选切换按钮
- 星标论文在文库中以金色星标标识

**断点续读：**
- `Paper.scrollPosition` 双精度字段
- 打开论文时自动恢复 `ScrollController` 位置
- 退出时自动保存当前滚动位置

**字体大小持久化：**
- `AppConfig.fontSize` 字段写入 SharedPreferences
- 设置页调整字体大小后立即可见，重启不丢失

**MinerU Agent API 降级链：**
- `pdf_fallback_service.dart` 重写为四层降级链
- Agent API（`/api/v1/agent/parse/file`，免 Key，≤20页/10MB）→ poppler → flutter_pdf → 元数据
- `parsePaper()` 中标准 API 失败后自动调用降级链

**Deep link 协议：**
- `paperpal://arxiv/XXXX` 自定义协议注册（Windows 安装时）
- `windows/runner/main.cpp` 解析命令行参数，启动时导航至对应论文
- 支持从浏览器点击 arxiv 链接直接打开

**Zotero 集成：**
- `lib/core/api/zotero_api.dart` — Zotero API 客户端（Dio）
- 导入对话框：选择列表 → 确认 → 自动下载 PDF + 元数据补全
- 环境变量配置：`ZOTERO_API_KEY` / `ZOTERO_USER_ID`

**高亮笔记：**
- `NoteType.highlight` 枚举值（+ `NoteType.question`）
- 阅读页划词 → 弹出操作菜单 → "添加高亮"
- 笔记面板按类型分色显示（高亮=琥珀色、笔记=主题色、问答=紫色）

**设置页改进：**
- `AppConfig` 模型字段扩展：`fontSize`、`defaultSoulId` 等
- 保存/加载状态反馈（SnackBar + loading indicator）
- 文库空状态导航按钮（一键跳转搜索页）

**性能与代码质量：**
- `AnimatedBackground` 子组件包裹 `RepaintBoundary`
- `flutter analyze`：从 356 降至 **0 issues**
- `use_build_context_synchronously` 修复 18 处（`mounted` 检查 + dispose guard）
- `test/` 中 `const` constructors 修复 **125 处**
- CLI 工具 `print()` → `stdout.writeln`
- `tool/` 未使用 import 清理

**Widget 测试覆盖（359→366）：**
- `avatar_picker_test.dart` — 头像选择器打开/关闭（2 测试）
- `explain_dialog_test.dart` — 公式解释弹窗渲染（1 测试）
- `soul_selector_test.dart` — 灵魂选择/切换/高亮/空状态（4 测试）

**Android 签名配置模板：**
- `android/key.properties.example` — 签名配置模板，含使用说明

### v0.4.2（2026-07-15）— 划词问答 + 文库排序/搜索 + batch import + dark mode + 重构

**划词问答：**
- 阅读页选中文本 → 弹出浮动操作栏 → QA 面板自动填入选中内容
- Shift+Enter 快速发送问答

**文库增强：**
- 排序：最后阅读 / 导入时间 / 标题
- 文库内搜索（filter 文本框）
- 已导入标记：搜索结果中显示 "已导入" 徽章
- 最后阅读时间显示 + 按最后阅读排序
- `touchPaper()` 自动更新 `lastReadAt`

**Batch import：**
- `file_picker` 多选模式批量导入 PDF
- 系统托盘增强：右键菜单新增 Quick Search + Import URL

**QA 面板改造：**
- 动态高度（auto-expand）+ 展开/折叠切换
- 导出按钮移至 AppBar

**UX 优化：**
- Dark mode 切换（导航栏新增暗色模式按钮）
- 重解析兜底（阅读页解析失败可重新发起 MinerU 解析）
- 导入完成后 "查看" 按钮
- 错误信息显示具体原因而非通用提示
- 设置页保存反馈
...
- 死代码路径清理
- fontSize 硬编码修复
- API Key 显示/隐藏 toggle 修复
- QA dispose `mounted` 守卫
- CLI help 格式化优化

**架构重构：**
- `SearchPage` QA 区 `setState` → StreamBuilder
- `LibraryPage` 筛选链重构（排序+搜索+状态分区）
- `read_page.dart` 模块拆分（QA panel 400+ 行 → 独立组件）

### v0.4.1（2026-07-15）— MinerU fallback + 错误处理优化

**MinerU fallback 基础：**
- 标准 API 解析失败时，自动尝试免 Token 的 Agent 轻量 API
- 以 `ParseResult` 统一输出格式，业务层无感知
- 文件上传 403 修复：PUT 时移除 Content-Type 头
...
### v0.4.0（2026-07-15）— 架构重构 + DesignTokens

**架构重构：**
- `DesignTokens` 系统：4px 网格，0 硬编码色值/间距
- `ServiceLocator` 接口化：所有服务通过接口类型注册
- `createLocator()` 共享初始化：CLI + Flutter 统一入口
- `lib/core/` 纯 Dart 化：所有 Flutter Widget 移入 `lib/ui/`

**测试提升：**
- 测试总数从 ~260 → 340
- Model 边界 case 全覆盖（43 测试）
- 服务边界 coverage 增强
