...
> 统一架构设计 v2.0 — 融合 MarkItDown (文档转换) + Kori (UI 风格 + AI 笔记) + MD Preview (阅读体验)

## 哲学

```
一个入口，万种文档，Kori 风格界面，有灵魂的 AI 伙伴。
```
...
| **Kori** | 干净卡片 UI + 笔记即写作 + 多格式渲染 | Kori 风格组件 + `TemplateService` + Mermaid 支持 |
...
│   │   └── zotero_service.dart         (NEW) Zotero 服务层
│   └── interfaces/
│       └── services.dart             (MODIFIED) 新接口 (IZoteroService, IMermaidRenderer)
├── ui/
│   ├── pages/
│   │   ├── read_page.dart            (MODIFIED) Mermaid + 搜索 + 高亮
│   │   ├── library_page.dart         (MODIFIED) Kori 风格卡片
│   │   ├── settings_page.dart        (MODIFIED) Kori 风格 + 7 主题
│   │   └── template_page.dart        (NEW) 笔记模板管理
│   └── widgets/
│       ├── mermaid_widget.dart       (NEW) Mermaid 组件
│       ├── find_bar.dart             (NEW) 页面内搜索
│       ├── template_picker.dart      (NEW) 模板选择器
│       ├── app_sidebar.dart          (NEW) Kori 风格侧边栏
│       └── adaptive_drawer.dart      (NEW) 响应式抽屉
├── theme/
│   └── themes/                       (NEW) 7 套 ColorScheme
└── tool/
    ├── markitdown_bridge.py          (NEW) Python 桥接脚本
    └── commands/
        └── convert_command.dart      (NEW) 文档转换 CLI
```
